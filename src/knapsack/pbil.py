...
def pbil(
    ...
    """
    items = sorted(items, key=lambda x: x.ratio, reverse=True) # sort items by value/weight ratio to match A* algorithm
    num_items = len(items)
    ...
    for i in range(1,num_generations+1):
        # generate population
        population = [Specimen(items, probability_vector, total_capacity) for _ in range(population_size)]
        ...
        for i in range(num_items):
            probability_vector[i] = (
                (1 - learning_rate) * probability_vector[i] +
